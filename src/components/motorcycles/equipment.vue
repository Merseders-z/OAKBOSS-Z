					...
					<el-table :data="tableData" stripe style="width: 100%">
						...
						<el-table-column prop="online" label="状态">
							<template slot-scope="scope">
								...
								<div v-else="scope.row.online == false">
									离线
								</div>
							</template>
						</el-table-column>
			...
			<div class="tubiao" v-else>
				<div class="tubiaoBox" v-for="item in tableData">
					...
					<div class="tubiaoBoxCon">
						...
						<div class="posWord2">机号:{{item.mac}}</div>
					...
					<div class="tubiaoBoxBot">
						<div class="tubiaoBoxBotImg">
							...
							<img src="../../assets/images/restart.png" style="width: 18px;height: 18px;cursor: pointer;"@click="reOn(item.id)" title="重启设备" v-if="item.online == true">
							<img src="../../assets/images/restart.png" style="width: 18px;height: 18px;cursor: pointer;" @click="chongqi()" title="重启设备" v-if="item.online == false">
						</div>
					</div>
				</div>
			</div>
		...
		<el-dialog title="添加设备" :visible.sync="addDialogVisible" width="30%" @close="addDialogClosed">
			<el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="80px">
				...
				<el-form-item label="地址" prop="address">
					<el-input v-model="addForm.address" class="addinput"></el-input>
				</el-form-item>
...
<script>
	...
	export default {
		...
		methods: {
			...
			chongqi(){
				this.$message.error('重启失败，设备已离线')
			},
			getImg() {
				//token去掉引号
				let toKen = this.token.replace(/\"/g, "")
				//console.log(toKen)
				this.$axios.get("admin/api/chargers/19?token=" + toKen + "&page=" + this.pagenum + "&row=15&attach=state&keyword=" + this.input)
					.then(res => {
						console.log(res.data)
						// console.log(res.status)//打印状态码
						if (res.status == 200) {
