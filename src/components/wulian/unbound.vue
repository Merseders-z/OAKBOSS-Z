<template>
	<div class="tem-right">
		<div class="tem-right-top">
			<div class="top-left">
				<div class="top-left-word">未绑定</div>
			</div>
			<!-- 设备确认 -->
						...
						<el-table :data="parkList" stripe style="width: 100%">
							...
							<el-table-column prop="update_time" label="创建日期">
								<template slot-scope="scope">
									<div style="text-align: center;">{{timeFormat(scope.row.update_time)}}</div>
								</template>
							</el-table-column>
					...
					<div v-else class="quans">
						<div class="moshi" v-for="item in qrlist" :key="item.id">
							...
							<div class="tow">
								...
								<img src="../../assets/images/QRbinding.png" style="width: 18px;height: 18px;cursor: pointer;" title="绑定设备" @click="choose(item.id,item.code,item.dev_name)"/>
...
<script>
	...
	export default {
		...
		methods: {
			...
			choose(id,code,dev_name){
				if(dev_name){
					this.$message.error('绑定失败,该二维码已被绑定!')
				}else{
					this.bangDialogVisible = true
				}
				this.chooseid = id
				this.qrcodeW = code
