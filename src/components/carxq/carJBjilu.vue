	...
	<div class="oneCard-right">
		...
		<div class="myhead2">
			<div class="myhead2-left">
				...
				<span class="jiben-nameWord">{{parkingName}}用户</span>
		...
		<div class="UserAssets-right-text">
			<div class="textBox">
				...
				<input v-model="input" placeholder="请输入关键字进行查找" class="textWord" clearable>
			</div>
			<div>
		...
		<div class="he">
			<template>
				<el-table :data="cardList" stripe style="width: 100%">
					...
					<el-table-column prop="enter_time" label="停车时间" width="180">
					</el-table-column>
...
<script>
	...
	export default {
		...
		created() {
			...
			this.parkingName = localStorage.getItem("carChargerName")
			this.parkingID = localStorage.getItem("carChargerID")
			this.getUserMes()
		},
		methods: {
			...
			getUserMes() {
				...
				this.$axios.get("admin/api/station/" + this.parkingID + "/pdrs?token=" + toKen + "&page=" + this.pagenum + "&row=11&keyword=" + this.input)
					.then(res => {
						console.log(res)
						if (res.status == 200) {
...
<style scoped="scoped">
	...
	.he>>>.el-table td, .el-table th.is-leaf{
		text-align: center;
		font-size: 15px;
	}
