								...
								<div class="leftBox" :class="{ BGactive:scope.row.id == isActive }"
									...
									<div class="leftBox-con" @click="getParksMes(scope.row.id),(scope.row.station)">
										...
										<div class="parkIcon">
											...
											<div class="orangeBox">
												<span class="parkIcon-word">{{scope.row.stat.busy}}/{{scope.row.stat.total}}</span>
											</div>
	...
	export default {
		...
		methods: {
			...
			getInputMes() {
				...
				let toKen = this.token.replace(/\"/g, "")
				// console.log(toKen)
				this.$axios.get("admin/api/stations?token=" + toKen + "&page=" + this.pagenum + "&row=6&keyword=" + this
						.input1)
					.then(res => {
						if (res.status == 200) {
