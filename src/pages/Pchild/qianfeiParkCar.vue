		...
		<div>
			<template>
				<el-table :data="cardList" stripe style="width: 100%">
					...
					<el-table-column prop="enter_devname" label="入口">
					</el-table-column>
					<el-table-column prop="leave_devname" label="出口">
					</el-table-column>
					<el-table-column prop="debts" label="欠费金额">
					</el-table-column>
					<el-table-column prop="station_name" label="停车场名称" show-overflow-tooltip>
					</el-table-column>
					<el-table-column prop="enter_time" label="停车时间" width="180" show-overflow-tooltip>
					</el-table-column>
					<el-table-column prop="leave_time" label="离场时间" width="180">
					</el-table-column>
					<el-table-column prop="address" label="操作" width="200">
						<template slot-scope="scope">
							<div class="operation">
								<div @click="jiedan(scope.row.id)">
									<img src="../../assets/images/statement.png" style="cursor: pointer;"/>
								</div>
								<div @click="gotopark(scope.row.id,scope.row.plate)">
									<img src="../../assets/images/see.png" style="cursor: pointer;"/>
								</div>
							</div>
						</template>
					</el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async jiedan(id){
				let toKen = this.token.replace(/\"/g, "")
				const confirmRes = await this.$confirm('确认结单吗?', '提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning'
				}).catch(err => err)
				// console.log(confirmRes)
				if (confirmRes !== 'confirm') {
					return this.$message.info('已取消结单')
				}
				this.$axios.post("admin/api/pdr/" + id + "/termination?token=" + toKen).then(res=>{
					console.log(res)
					if (res.status == 200) {
