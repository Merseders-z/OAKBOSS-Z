	...
	<div class="oneCard-right">
		...
		<div class="paibanBox">
			<div class="paibanLeft">
				<div class="PLTbox">
					<el-button type="primary" icon="el-icon-plus" @click="addDialogVisibleClick">添加</el-button>
			...
			<div class="paibanRight">
				<div class="PRTbox">
					<div class="dateBox" v-for="item in week" :class="{BGactive:isActive == item.id}"
						@click="changeBg(item.id,item.num)" :key="item.id">
						{{item.name}}
					</div>
				</div>
				<div class="PRBbox">
		...
		<!-- 添加设备 -->
		<el-dialog title="添加排班" :visible.sync="addDialogVisible" width="30%">
					...
					<div class="plateBox" v-show="flag == false">
						<div class="boweiBox">
							<div class="bowei" v-for="item in ax">
								<div style="display: flex;flex-direction: row;justify-content: flex-end;width: 90%;font-size: 15px;margin: 0 auto;cursor: pointer;"
									@click="shanbowei(item.id)">X</div>
	...
	export default {
		...
		methods: {
			...
			addDialogVisibleClick() {
				...
				this.addForm.date = this.isActive
			},
			changeBg(id, num) {
				this.isActive = id;
				this.w = num
			...
			xiugai() {
				let toKen = this.token.replace(/\"/g, "")
				this.$axios.put("/admin/api/administrator/" + this.chooseSFYID + "/workgroup/" + this.isActive1 +
					"/schedules/" + this.isActive, {
						token: toKen,
