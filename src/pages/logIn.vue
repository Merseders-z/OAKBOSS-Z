<template>
	<div class="login_main">
			...
			<div class="wrap tabwrap">
				<div class="module" v-if="isgo">
					<div class="login_form">
						<li class="item">
							<input type="text" class="ipt ipt_txt ipt_user" v-model="num" auto-complete="off"
								prop="mobile" placeholder="请输入您的账号" />
						</li>
						<li class="item">
							<input type="password" class="ipt ipt_txt ipt_psd" v-model="password" auto-complete="off"
								prop="mobile" placeholder="请输入您的密码" @keyup.enter="huiche" />
						</li>
						<div class="bot">
				...
				<div class="module" v-if="isgoss">
					<div class="login_form">
						...
						<li class="item fi">
							<input type="text" autocomplete="new-password" class="ipt ipt_txt yanzheng" v-model="cod"
								placeholder="请您输入验证码" />
		...
		<div class="jianyiword">建议使用Chrome浏览器，分辨率1920X1080，操作系统字体100%</div>
					...
					.then((res) => {
						...
						if (res.data.error == 0) {
							if (this.checked == true) {
								...
								if (x.length == 3) {
									if (res.data.admin) {
										localStorage.setItem("token", JSON.stringify(this.numberMsg.admin.token));
										localStorage.setItem("admin", JSON.stringify(this.numberMsg.admin));
										console.log(this.numberMsg.admin.token, 1234)
										this.$router.push({
											path: "/summary",
										});
									}else if (res.data.agent) {
										localStorage.setItem("token", JSON.stringify(this.numberMsg.agent.token));
										localStorage.setItem("admin", JSON.stringify(this.numberMsg.agent));
										console.log(this.numberMsg.agent.token, 4321)
										this.$router.push({
											path: "/summary",
										});
									}
								} else {
									localStorage.setItem("adminMes", JSON.stringify(this.numberMsg.admin));
									localStorage.setItem("agentMes", JSON.stringify(this.numberMsg.agent));
									localStorage.setItem("token", JSON.stringify(this.numberMsg.admin.token));
									localStorage.setItem("agentToken", JSON.stringify(this.numberMsg.agent.token));
									this.$router.push({
										path: "/loginChoose",
									});
								}
