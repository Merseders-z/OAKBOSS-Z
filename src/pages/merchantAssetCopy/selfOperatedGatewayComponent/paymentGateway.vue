<template>
	<div class="oneCard-right">
		<div class="UserAssets-right-top">
			<div class="user-left">
				<span class="user-word">支付网关</span>
			</div>
					...
					<el-table :data="tableData" stripe style="width: 100%">
						...
						<el-table-column show-overflow-tooltip prop="gateway" label="网关">
							<template slot-scope="scope">
								<div v-if="scope.row.gateway == 'alipay'">
									支付宝
								</div>
								<div v-else-if="scope.row.gateway == 'wechat'">
									微信
								</div>
								<div v-else-if="scope.row.gateway == 'abc'">
									农行
								</div>
								<div v-else-if="scope.row.gateway == 'hxb'">
									华夏银行
								</div>
								<div v-else-if="scope.row.gateway == 'userpay'">
									钱包
								</div>
								<div v-else-if="scope.row.gateway == 'cash'">
									现金
								</div>
						...
						<el-table-column prop="address" label="操作">
							<template slot-scope="scope">
								<div class="operation">
									<div>
										<button type="text" @click="removeUserByID(scope.row.id)"><img src="../../../assets/images/delete.png" style="cursor: pointer;" title="删除"/></button>
									</div>
									<div>
										<button type="text" @click="showEditDialog(scope.row.id)"><img src="../../../assets/images/compile.png" style="cursor: pointer;" title="修改"/></button>
									</div>
								</div>
							</template>
						</el-table-column>
			...
			<div class="tubiao" v-else>
				<div class="tubiaoBox" v-for="item in tableData">
					<div class="tubiaoBoxTop">
						...
						<div class="xinhao">
							<img src="../../../assets/images/NoSignalX.png" style="width: 41px;height: 31px;" v-show="item.online == false">
							<img src="../../../assets/images/Full signal.png" v-show="item.online == true&&item.state.signal>75">
							<img src="../../../assets/images/signal3.png" v-show="item.online == true&&item.state.signal<75&&item.state.signal>50">
							<img src="../../../assets/images/signal2.png" v-show="item.online == true&&item.state.signal<50&&item.state.signal>20">
							<img src="../../../assets/images/signal1.png" v-show="item.online == true&&item.state.signal<25&&item.state.signal>0">
							<img src="../../../assets/images/TemporarilyNoData.png" style="width: 41px;height: 31px;" v-show="online == true">
						</div>
					</div>
					...
					<div class="tubiaoBoxBot">
						<div class="tubiaoBoxBotImg">
							<img src="../../../assets/images/Tdelete.png" style="width: 20px;height: 20px;cursor: pointer;" @click="removeUserByID(item.id)" title="删除">
							<img src="../../../assets/images/Teditor.png" style="width: 18px;height: 18px;cursor: pointer;"@click="showEditDialog(item.id)" title="修改">
		...
		<el-dialog title="添加支付网关" :visible.sync="addDialogVisible" width="30%" @close="addDialogClosed">
			<el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="80px">
				...
				<el-form-item label="appid" prop="appid">
					<el-input v-model="addForm.appid" class="addinput"></el-input>
				</el-form-item>
		...
		<el-dialog title="修改设备" :visible.sync="editDialogVisible" width="30%" @close="editDialogClosed">
			<el-form :model="editForm" ref="editFormRef" label-width="80px">
				...
				<el-form-item label="appid" prop="appid">
					<el-input v-model="editForm.appid" class="addinput"></el-input>
				</el-form-item>
				<el-form-item label="网关" prop="gateway">
	...
	export default {
		...
		methods: {
			...
			getUserMes() {
				...
				//console.log(toKen)
				this.$axios.get("/admin/api/paygws?token=" + toKen + "&page=" + this.pagenum + "&row=14&keyword=" + this.input)
					.then(res => {
						console.log(res.data)
						// console.log(res.status)//打印状态码
						if (res.status == 200) {
							this.tableData = res.data.paygws //用户列表数据
			...
			getImg() {
				...
				let toKen = this.token.replace(/\"/g, "")
				//console.log(toKen)
				this.$axios.get("/admin/api/paygws?token=" + toKen + "&page=" + this.pagenum2 + "&row=15&attach=state&keyword=" + this.input)
					.then(res => {
						console.log(res.data)
						// console.log(res.status)//打印状态码
						if (res.status == 200) {
			...
			async removeUserByID(id) {
				...
				this.$axios.delete("/admin/api/paygw/" + id + "?token=" + toKen)
					.then(res => {
			...
			chargersUser() {
				...
				this.$refs.addFormRef.validate(valid => {
					if (!valid) {
						...
						this.$axios.post("admin/api/paygw", this.addForm)
							.then(res => {
								if (res.status !== 200) {
			...
			showEditDialog(id) {
				...
				let toKen = this.token.replace(/\"/g, "")
				// console.log(toKen)
				this.$axios.get("admin/api/paygw/" + id + "?token=" + toKen) //根据id点击修改的id查询设备信息
					.then(res => {
						//console.log(res.status)
						if (res.status == 200) {
							this.editForm = res.data.paygw
							console.log(res.data.paygw)
						}
					})
			},
			editChargerInfo() {
				...
				this.$refs.editFormRef.validate(valid => {
					if (!valid) {
						...
						if (confirm("确认修改吗?")) {
							this.$axios.put("admin/api/paygw/" + this.editForm.id, {
									token: toKen,
