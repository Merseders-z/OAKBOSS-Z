<template>
	<div class="oneCard-right">
		<div class="UserAssets-right-top">
			<div class="user-left">
				<span class="user-word">发票网关</span>
			</div>
	...
	export default {
		...
		methods: {
			...
			getUserMes() {
				...
				//console.log(toKen)
				this.$axios.get("/admin/api/receipt_gws?token=" + toKen + "&page=" + this.pagenum + "&row=14&keyword=" + this.input)
					.then(res => {
						console.log(res.data)
						// console.log(res.status)//打印状态码
						if (res.status == 200) {
							this.tableData = res.data.receipt_gws //用户列表数据
			...
			getImg() {
				...
				let toKen = this.token.replace(/\"/g, "")
				//console.log(toKen)
				this.$axios.get("/admin/api/receipt_gws?token=" + toKen + "&page=" + this.pagenum2 + "&row=15&attach=state&keyword=" + this.input)
					.then(res => {
						console.log(res.data)
						// console.log(res.status)//打印状态码
						if (res.status == 200) {
			...
			async removeUserByID(id) {
				...
				this.$axios.delete("/admin/api/receipt_gw/" + id + "?token=" + toKen)
					.then(res => {
			...
			chargersUser() {
				...
				this.$refs.addFormRef.validate(valid => {
					if (!valid) {
						...
						this.$axios.post("admin/api/receipt_gw", this.addForm)
							.then(res => {
								if (res.status !== 200) {
			...
			showEditDialog(id) {
				...
				let toKen = this.token.replace(/\"/g, "")
				// console.log(toKen)
				this.$axios.get("admin/api/receipt_gw/" + id + "?token=" + toKen) //根据id点击修改的id查询设备信息
					.then(res => {
						//console.log(res.status)
						if (res.status == 200) {
							this.editForm = res.data.receipt_gw
							console.log(res.data.receipt_gw)
						}
					})
			},
			editChargerInfo() {
				...
				this.$refs.editFormRef.validate(valid => {
					if (!valid) {
						...
						if (confirm("确认修改吗?")) {
							this.$axios.put("admin/api/receipt_gw/" + this.editForm.id, {
									token: toKen,
