		...
		<div>
			<template>
				<el-table :data="cardList" stripe style="width: 100%">
					...
					<el-table-column prop="leave_time" label="驶离时间">
					</el-table-column>
					...
					<el-table-column prop="" label="停车费(已缴/总费用)" width="200">
						<template slot-scope="scope">
							<div>{{scope.row.paid}}/{{scope.row.amount}}</div>
						</template>
					</el-table-column>
					<!-- <el-table-column prop="exception" label="异常原因">
						<template slot-scope="scope">
							<div>{{cause(scope.row.exception)}}</div>
						</template>
					</el-table-column> -->
	...
	export default {
		...
		methods: {
			...
			getUserMes() {
				...
				this.$axios.get("admin/api/exception/pdrs?token=" + toKen + "&page=" + this.pagenum + "&row=14&keyword=" + this.input+"&from=" + this.startTime + "&to=" + this.endTime)
					.then(res => {
