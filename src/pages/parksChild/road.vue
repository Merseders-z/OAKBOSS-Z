	...
	<div class="tem-right">
			...
			<div class="right-con-left">
				<div class="right-con-top">
					<div class="textBox">
						...
						<el-input placeholder="请输入关键字查找" class="textWord" v-model="input1" clearable
						...
						<el-table-column show-overflow-tooltip>
							<template slot-scope="scope">
								<div class="leftBox" :class="{ BGactive:scope.row.id == isActive }"
									...
									<div class="parkimg">
										<img src="../../assets/images/roadparking.png" />
									</div>
									<div class="leftBox-con" @click="getParksMes(scope.row.id),(scope.row.station)">
										...
										<div class="parkIcon">
											<div class="orangeBox">
												<span class="parkIcon-word">路边停车</span>
											</div>
											<div class="orangeBox">
												<span class="parkIcon-word">{{scope.row.stat.busy}}/{{scope.row.stat.idle}}</span>
											</div>
											<div class="orangeBox1">
												<!-- <span class="parkIcon-word">{{scope.row.price}}</span> -->
											</div>
										</div>
									</div>
									<template v-if="scope.row.id == isActive">
										<div class="leftBox-right">
											<div @click="showEditDialog1(scope.row.id)">
												<img src="../../assets/images/xiu.png" title="修改" style="cursor: pointer;">
											</div>
											<div @click="removeUserByID1(scope.row.id)">
												<img src="../../assets/images/shan2.png" title="删除" style="cursor: pointer;">
											</div>
											<div
												@click="huodeid(scope.row.id,scope.row.name,scope.row.address,scope.row.type)">
												<img src="../../assets/images/lan.png" title="查看" style="cursor: pointer;">
											</div>
										</div>
									</template>
									<template v-else>
			...
			<div class="right-con-right">
				<div class="sousuo">
					<div class="textBox-right">
						...
						<el-input placeholder="请输入关键字进行查找" v-model="input2" class="textWord"
							@keyup.enter.native="getRoadChargers" clearable></el-input>
					</div>
				...
				<template>
					<el-table :data="parkList" stripe style="width:100%">
						...
						<el-table-column prop="dev_id" label="泊位号">
						</el-table-column>
						<el-table-column prop="mac" label="地磁">
						</el-table-column>
						<el-table-column prop="busy" label="状态">
							<template slot-scope="scope">
								<div v-if="scope.row.busy == true">
									占用
								</div>
								<div v-if="scope.row.busy == false">
									空闲
								</div>
							</template>
						</el-table-column>
								...
								<div class="operation">
									<div>
										<button type="text" @click="removeUserByID(scope.row.id)"><img
												src="../../assets/images/delete.png" style="cursor: pointer;"/></button>
									</div>
									<div>
										<el-button type="text" @click="showEditDialog(scope.row.id)"><img
												src="../../assets/images/compile.png" style="cursor: pointer;"/></el-button>
									</div>
									<!-- <div>
										<img style="height:14px;width:19px;cursor: pointer;" src="../../assets/images/see.png" >
									</div> -->
								</div>
							</template>
						</el-table-column>
		...
		<el-dialog title="添加设备" :visible.sync="addDialogVisible" width="30%" @close="addDialogClosed">
			<el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="80px">
				...
				<el-form-item label="地磁" prop="mac">
...
<script>
	...
	export default {
		...
		methods: {
			huodeid(id, name,address,type) {
				localStorage.setItem("carChargerID",id);
				localStorage.setItem("carChargerName",name);
				localStorage.setItem("carChargerAddress",address);
			...
			getInputMes() {
				...
				let toKen = this.token.replace(/\"/g, "")
				// console.log(toKen)
				this.$axios.get("admin/api/stations/3?token=" + toKen + "&page=" + this.pagenum + "&row=6&keyword=" + this
						.input1)
					.then(res => {
						if (res.status == 200) {
			...
			getParksMes(id) {
				...
				this.leftType = id
				let toKen = this.token.replace(/\"/g, "")
				this.$axios.get("admin/api/station/" + id + "/chargers?token=" + toKen + "&page=" + this.newpark +
						"&row=14")
					.then(res => {
						//console.log(id)
						console.log(res)
			...
			getRoadChargers() {
				...
				let toKen = this.token.replace(/\"/g, "")
				// console.log(toKen)
				this.$axios.get("admin/api/station/" + this.getFristID + "/chargers?token=" + toKen + "&page=" + this
						.newpark +
			...
			chargersUser() {
				...
				this.$refs.addFormRef.validate(valid => {
					if (!valid) {
						...
						this.$axios.post("/admin/api/charger?token=" + this.addForm.token + "&dev_id=" + this
								.addForm.dev_id + "&name=" +
								this.addForm.name + "&mac=" + this.addForm.mac + "&type=" + this.value +
								"&station=" + this.getFristID)
							.then(res => {
								if (res.status !== 200) {
