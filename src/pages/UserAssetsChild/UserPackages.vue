	...
	export default {
		...
		methods: {
			...
			getUserMes() {
				...
				// console.log(toKen)
				this.$axios.get("/admin/api/user/plans/1?token=" + toKen + "&page=" + this.pagenum + "&row=14&keyword=" +this.input)
					.then(res => {
			...
			getImg() {
				...
				let toKen = this.token.replace(/\"/g, "")
				// console.log(toKen)
				this.$axios.get("/admin/api/user/plans/1?token=" + toKen + "&page=" + this.pagenum2 + "&row=10&keyword=" +this.input)
					.then(res => {
						// console.log(res.data.users)
						// console.log(res.status)//打印状态码
						if (res.status == 200) {
