		...
		<div class="kanbanCon">
			...
			<div class="kanbanConBox" v-show="flag == 1">
				<div class="kanbanConTop" v-for="item in numlist">
					...
					<div class="kanbanConTopWord">
						<div class="kanbanConTopW1">支付宝用户数</div>
						<div class="kanbanConTopW2">{{item.users_type_3_count}}</div>
					</div>
					<div class="kanbanConTopWord">
	...
	export default {
		...
		methods: {
			...
			chooseYM(id) {
				...
				if (id == 2) {
					this.$axios.get("admin/api/report/DE24EADC179A4D2BA5799A4916C61878?token=" + toKen +
							"&order=dt&sort=desc&page=1&row=1")
						.then(res => {
					...
					this.$axios.get("admin/api/report/6B52A176C758419699BC8E2113094232?token=" + toKen +
							"&order=dt&sort=desc&page=1&row=7&from=" + this.value1 + "&to=" + this.value2)
						.then(res => {
							console.log(res.data.data)
							if (res.status == 200) {
			...
			searchBot4() {
				...
				if (this.isBg2 == 1) {
					...
					this.$axios.get("admin/api/report/0686D43D0CFE4EB8955BAB813B663FAD?token=" + toKen +
							"&order=dt&sort=desc&page=1&row=30&from=" + this.value1 + "&to=" + this.value2)
						.then(res => {
							console.log(res.data.data)
							if (res.status == 200) {
								this.zhexian4 = res.data.data
					...
					this.$axios.get("admin/api/report/74AD058E9AB04435AD2AD77F1F02846B?token=" + toKen +
							"&order=dt&sort=desc&page=1&row=12&from=" + this.value1 + "&to=" + this.value2)
						.then(res => {
							console.log(res.data.data)
							if (res.status == 200) {
								this.zhexian4 = res.data.data
			...
			fristMes() {
				let toKen = this.token.replace(/\"/g, "")
				this.$axios.get("admin/api/report/C66DF2CDB1224F1ABFCE23D2412A6BA5?token=" + toKen +
						"&order=dt&sort=desc&page=1&row=7")
					.then(res => {
						console.log(res.data.data)
						if (res.status == 200) {
							this.zhexianList = res.data.data
			...
			getDateMes1(id) {
				...
				if (id == 1) {
					...
					this.$echarts.init(document.getElementById("myEcharts4")).dispose();
					this.$axios.get("admin/api/report/0686D43D0CFE4EB8955BAB813B663FAD?token=" + toKen +
							"&order=dt&sort=desc&page=1&row=30")
						.then(res => {
							console.log(res.data.data)
							if (res.status == 200) {
								this.zhexian4 = res.data.data
					...
					this.$echarts.init(document.getElementById("myEcharts4")).dispose();
					this.$axios.get("admin/api/report/74AD058E9AB04435AD2AD77F1F02846B?token=" + toKen +
							"&order=dt&sort=desc&page=1&row=12")
						.then(res => {
							console.log(res.data.data)
							if (res.status == 200) {
								this.zhexian4 = res.data.data
