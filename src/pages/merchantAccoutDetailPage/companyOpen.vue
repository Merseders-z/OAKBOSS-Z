<template>
	<div class="oneCard-right">
		<div class="UserAssets-right-top">
			<div class="user-left">
				<span class="user-word">放开平台</span>
			</div>
		...
		<div class="he">
			<template>
				<el-table :data="cardList" stripe style="width: 100%">
					...
					<el-table-column prop="partner" show-overflow-tooltip label="合作方ID">
					</el-table-column>
	...
	export default {
		...
		methods: {
			...
			getUserMes() {
				...
				console.log(userID)
				this.$axios.get("admin/api/agent/" + userID + "/partners?token=" + toKen + "&page=" + this.pagenum +
						"&row=11&keyword=" + this.input)
					.then(res => {
						console.log(res)
						if (res.status == 200) {
							this.cardList = res.data.partners
			...
			async removeUserByID(id) {
				...
				this.$axios.delete("admin/api/partner/" + id + "?token=" + toKen)
					.then(res => {
			...
			showEditDialog(id) {
				...
				let toKen = this.token.replace(/\"/g, "")
				// console.log(toKen)
				this.$axios.get("admin/api/partner/" + id + "?token=" + toKen) //根据id点击修改的id查询设备信息
					.then(res => {
						//console.log(res.status)
						if (res.status == 200) {
							this.editForm = res.data.partner
			...
			editChargerInfo() {
				...
				this.$refs.editFormRef.validate(valid => {
					if (!valid) {
						...
						if (confirm("确认修改吗?")) {
							this.$axios.put("admin/api/partner/" + this.editForm.id, {
									token: toKen,
			...
			addPartner() {
				let toKen = this.token.replace(/\"/g, "")
						...
						this.$axios.post("/admin/api/partner", this.addForm)
							.then(res => {
								if (res.status !== 200) {
